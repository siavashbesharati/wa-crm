...
| **Docker** | Engine **24.0+** + Compose v2 | Bundled `docker-compose.yml` and `docker-compose.override.yml` |
...
them in `docker-compose.override.yml` or as a `deploy.resources` block per
service.
...
6. ✅ **Set resource limits** in `docker-compose.override.yml` so a single
...
- `docker-compose.override.yml` — dev override (live-reload)
