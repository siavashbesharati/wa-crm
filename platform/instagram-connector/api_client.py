...
class ApiClient:
    ...
    def list_sessions(self) -> list[dict[str, Any]]:
        r = self._s.get(self._url("/internal/instagram/sessions"), timeout=20)
        r.raise_for_status()
        data = r.json()
        return data if isinstance(data, list) else []

    def get_auth(self, account_id: str) -> dict[str, Any]:
        r = self._s.get(self._url(f"/internal/instagram/sessions/{account_id}/auth"), timeout=20)
        r.raise_for_status()
        return r.json()
    ...
    def put_pair_state(
        ...
        *,
        ...
        r = self._s.put(
            self._url(f"/internal/instagram/sessions/{account_id}/pair-state"),
            params={
    ...
    def heartbeat(self, account_id: str) -> None:
        r = self._s.post(self._url(f"/internal/instagram/sessions/{account_id}/heartbeat"), timeout=15)
        r.raise_for_status()

    def ingest(self, account_id: str, payload: dict[str, Any]) -> dict[str, Any]:
        r = self._s.post(
            self._url(f"/internal/instagram/sessions/{account_id}/ingest"),
            json=payload,
            timeout=45,
        )
        r.raise_for_status()
    ...
    def claim_jobs(self, account_id: str, limit: int = 5) -> list[dict[str, Any]]:
        r = self._s.post(
            self._url("/internal/instagram/jobs/claim"),
    ...
    def complete_job(self, job_id: str, *, ok: bool = True, error: str = "") -> None:
        r = self._s.post(
            self._url(f"/internal/instagram/jobs/{job_id}/complete"),
