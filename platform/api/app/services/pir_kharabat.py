...
def coach_system_prompt() -> str:
    return (
        ...
        "همیشه خروجی را با Markdown بنویس تا در چت رندر شود: "
        "عنوان با ## یا ###، لیست با - یا ۱.، تاکید با **ضخیم**، کد/شناسه با `بک‌تیک`. "
        "از جدول Markdown فقط وقتی داده جدولی لازم است استفاده کن. "
        "متن خام بدون ساختار نفرست؛ پاراگراف‌ها را با خط خالی جدا کن. "
        "اگر «### گزارش عامل» یا «### گزارش تحلیلی» یا «### لیدهای واقعی» هست، "
        "نام‌ها و نتایج را از همان‌جا بگو؛ جعل نکن. "
        "هرگز نگو کانبان — بگو بورد لیدها یا صفحه وظایف. "
        "اگر اقدامی انجام شده (وظیفه/اختصاص/مرحله)، تأیید کوتاه بده و قدم بعدی پیشنهاد کن."
    )
...
def run_coach_turn(
    ...
    *,
    ...
    try:
        ...
        # org.id from auth session only — never from client body
        agent_report = run_agent_for_message(
            db, org_id=org.id, message=text, user_id=user_id
        )
        if agent_report:
            agent_bits = "\n\n" + agent_report
    except Exception:  # noqa: BLE001
    ...
    user_prompt = (
        ...
        f"### سوال کاربر تیم\n{text}\n\n"
        f"پاسخ {COACH_NAME} را فقط با Markdown معتبر بنویس "
        f"(عنوان، لیست، **ضخیم**؛ بدون HTML خام):"
    )
